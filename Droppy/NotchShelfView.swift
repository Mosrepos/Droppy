...
struct NotchShelfView: View {
    ...
    private var shouldDisableInlineNotchVisibilityAnimation: Bool {
        let unlockHandoffCondition = isLockHandoffActive &&
            hasPhysicalNotchOnDisplay &&
            isBuiltInDisplay

        return (lockScreenManager.isDedicatedHUDActive &&
                hasPhysicalNotchOnDisplay &&
                isBuiltInDisplay) ||
            unlockHandoffCondition
    }
    ...
    private func startAutoShrinkTimer() {
        ...
        if hotkeyGraceDelay > delay {
            DispatchQueue.main.asyncAfter(deadline: .now() + hotkeyGraceDelay, execute: workItem)
            return
        }
        DispatchQueue.main.asyncAfter(deadline: .now() + delay, execute: workItem)
    }
